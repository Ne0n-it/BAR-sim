...
TIME_LIMIT = 5 * 60        # 5 Minuti
EVAL_INTERVAL = 900         # Pruning ogni 30s
WIND_LEVEL = 14            # Vento medio
MEX_METAL_YIELD = 2.0
MEX_DISTANCES = [0, 5, 5, 10, 10, 15, 15, 20, 20, 25]

# Coda di costruzione iniziale (squad_idx, action_type, unit_name)
# action_type: 0 per build
INITIAL_BUILD_QUEUE = [
    ('Commander', 0, 'Mex'),
    ('Commander', 0, 'Solar'),
...
class BarSimOptimized:
    ...
    def branching_step(self):
        ...
        if cp.any(is_mex_action):
            parent_mex_action_idxs = parent_idxs[is_mex_action]

            parent_mex_counts = self.inv_buf[parent_mex_action_idxs, mex_idx]
            parent_adv_mex_counts = self.inv_buf[parent_mex_action_idxs, adv_mex_idx]
            total_mex_counts = parent_mex_counts + parent_adv_mex_counts

            valid_counts = cp.minimum(total_mex_counts, len(MEX_DISTANCES) - 1)

            # Use CuPy array for travel times directly on the GPU
            travel_times_gpu = cp.asarray(MEX_DISTANCES, dtype=cp.float32)
            waits[is_mex_action] = travel_times_gpu[valid_counts]
